...
fn anything_that_ends_with_ast_must_impl_the_string_before_ast(
    schema_text: &str,
) -> Vec<miette::Report> {
    let top_level_definitions = Parser::new(schema_text).parse().document();
    ...
    struct Output {
        ...
        implements_span_of_this: String,
    }
    ...
    execute_query(
        ...
        r#"
        {
            Schema {
                vertex_type {
                    name @tag @output(name: "what_to_implement") @filter(op: "not_has_suffix", value: ["$ast"])
                }

                vertex_type {
                    name @filter(op: "has_suffix", value: ["$ast"])
                         @filter(op: "has_prefix", value: ["%name"])
                         @output(name: "implements_span_of_this")
    ...
    .map(|row| {
        let Output {
            what_to_implement,
            implements_span_of_this,
        } = row.try_into_struct::<Output>().expect("incorrect shape");
        let object_type_name_token = top_level_definitions
            .definitions()
            .filter_map(|x| {
                let Definition::ObjectTypeDefinition(otd) = x else {
                    return None;
            ...
            .filter(|x| x.name().unwrap().text() == implements_span_of_this)
            // .map(|x| x.name().unwrap().ident_token().unwrap().text_range()) // for name span
            .map(|x| {
                x.implements_interfaces()
                    .unwrap()
                    .implements_token()
                    .unwrap()
                    .text_range()
        ...
        miette!(
            code = "oxc::implement_x_for_y",
            labels = vec![LabeledSpan::at(
                st..end,
                format!("implement {}", what_to_implement)
            )],
            "types that end with 'AST' must implement the string before the AST"
        )
    })
    .collect::<Vec<_>>()
}
...
fn main() -> Result<()> {
    ...
    // array_type_must_have_bang(schema_text) // <- should only work on edges
    // bottom_heavy_fields(schema_text) // <- sorting that we don't need now
    ...
    let errs = anything_that_ends_with_ast_must_impl_astnode(schema_text)
        ...
        .chain(anything_that_ends_with_ast_must_impl_the_string_before_ast(
            schema_text,
        ))
